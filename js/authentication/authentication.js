...
function resetaSenha() {
  ...
  firebase
    ...
    .catch((error) => {
      alerta(
        `❌ | Não foi possível enviar o email de redefinição de senha <br> ${
          errors[error.code]
        }`,
        "danger"
      );
...
function enviarEmailVerificacao() {
  ...
  user
    ...
    .catch((error) => {
      alerta(
        `❌ | Não foi possível enviar o email de verificação <br> ${
          errors[error.code]
        }`,
        "danger"
      );
...
function novoUsuario(email, senha) {
  firebase
    ...
    .then((userCredential) => {
      ...
      // alerta(`✅ | Usuário cadastrado !!`, 'success')
    ...
    .catch((error) => {
      ...
      alerta(
        `❌ | Não foi possível cadastrar o usuário <br> ${errors[error.code]}`,
        "danger"
      );
    });
}
...
function verificaLogado() {
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      user.photoURL
        ? (imagemUsuario.innerHTML = `<img src="${user.photoURL}" alt="Foto do Usuário" title="${user.displayName}" class="img rounded-circle pe-2 .d-sm-none" width="48" />`)
